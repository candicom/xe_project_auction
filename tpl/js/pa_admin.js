/**
 * @file   modules/pa/js/pa_admin.js
 * @author NHN (developers@xpressengine.com)
 * @brief  pa 모듈의 관리자용 javascript
 **/


/* complete to insert pa module */
function completeInsertPa(ret_obj) {
    ...
    var url = current_url.setQuery('act','dispPaAdminPaInfo');
    if(module_srl) url = url.setQuery('module_srl',module_srl);
    if(page) url.setQuery('page',page);
    location.href = url;
}

/* delete the pa module*/
function completeDeletePa(ret_obj) {
    var error = ret_obj['error'];
    var message = ret_obj['message'];
    var page = ret_obj['page'];
    alert(message);

    var url = current_url.setQuery('act','dispPaAdminContent').setQuery('module_srl','');
...
function completeUpdateCategory(ret_obj) {
    ...
    var url = current_url.setQuery('module_srl',module_srl).setQuery('act','dispPaAdminCategoryInfo');
    if(page) url.setQuery('page',page);
    location.href = url;
}
...
function doSaveListConfig(module_srl) {
    ...
    exec_xml('pa','procPaAdminInsertListConfig', params, function() { location.reload(); });
}
